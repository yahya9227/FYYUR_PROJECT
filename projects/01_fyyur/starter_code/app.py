...
def venues():
  ...
  for city in cities:
      ...
      for venue in venues:   
          new_upcoming_shows = venue.shows.filter(Show.start_time > current_time).all()  
          if current_city != venue.city:
                ...
                "id": venue.id,
                "name":venue.name,
                "num_upcoming_shows": len(new_upcoming_shows)}]
                })
...
def show_venue(venue_id):
  ...
  for show in upcoming_shows_query:
             if len(upcoming_shows_query) == 0:
                  upcoming_shows.update({"upcoming_shows_query": [],})
             else:
  ...
  for show in past_shows_query:
            if len(past_shows_query) == 0:
                  past_shows.update({"past_shows_query": [],})
            else:
                  artist = db.session.query(Artist.name, Artist.image_link).filter(Artist.id == show.artist_id).one()
                  past_shows.append({
                  "artist_id": show.artist_id,
  ...
  data.update({"past_shows_count": len(past_shows_query), "upcoming_shows_count": len(upcoming_shows_query),})
...
def edit_artist_submission(artist_id):
  ...
  try:
    ...
    # by using request.form.get is safer than accessing the value directly to handel null cases
    data.name = request.form.get('name') 
    data.city = request.form.get('city')
    data.state = request.form.get('state')
    data.phone = request.form.get('phone')
